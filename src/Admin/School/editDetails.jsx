...
class AddPage extends Component {
  ...
  async componentDidMount() {
    const { fetchClasses, fetchSubjects, fetchTeachers } = this.props;
    await fetchClasses();
    await fetchSubjects();
    await fetchTeachers();
  }
  ...
  createProperty = (values, e) => {
    e.preventDefault();
    const { patchSchool, eachData, fetchData, onCloseModal } = this.props;
  ...
  render() {
    ...
    const values = { classes, classType, subject, teacher, name };
    const { eachData } = this.props;
    return (
      <>
        <div>
          <Card>
            <CardHeader title="Edit" style={{ color: "#2196f3" }} />
