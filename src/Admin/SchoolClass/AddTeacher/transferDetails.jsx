...
class AddPage extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (posting.postTransfer !== prevProps.posting.postTransfer) {
      const { postTransfer } = posting;
      const { success } = postTransfer;
      if (success === false) {
        this.setState({
          snackBarOpen: true,
          snackBarVariant: "error",
          snackBarMessage: postTransfer.message,
          loading: false,
        });
        return false;
      }
  ...
  createProperty = (values, e) => {
    ...
    switch (pageType) {
      case "transfer":
        postTransfer(values, schFromID, teacher._id);
        this.setState({
          loading: true,
        });
        this.clearState();
            ...
            <CardContent>
              <div style={{ textAlign: "center", justifyContent: "center" }}>
                <form onSubmit={this.createProperty.bind(null, values)}>
                  ...
                  <h3 style={{ color: "#2196f3" }}>Transfer Details</h3>
